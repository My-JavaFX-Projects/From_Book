package org.example.tictactoe.clientTwo;
...
public class TicTacToeClient extends Application implements TicTacToeConstants {
    ...
    public void connectToServer() {
        try {
            Socket socket = new Socket(host, 8073);
    ...
    private void recieveInfoFromServer() throws IOException {
        ...
        if (status == PLAYER1_WON) {
            ...
            if (myToken == 'X') {
                ...
                Platform.runLater(() -> lblStatus.setText("Player 1 (X) won !"));
                recieveMove();
            }
            ...
            if (myToken == 'O') {
                ...
                Platform.runLater(() -> lblStatus.setText("Player 2 (O) won !"));
                recieveMove();
            }
        } else if (status == DRAW) {
            continueToPlay = false;
            Platform.runLater(() -> lblStatus.setText("Game is over, no winner"));
            if (myToken == 'O') recieveMove();
        }else {
            recieveMove();
